...
| 功能 | 说明 |
|------|------|
| 🤖 卡片自动生成 | 从大纲+拆文库一键生成角色和世界设定，自动跳过重名 |
| 📖 卡片从章节提取 | 从选定章节正文提取角色和设定 |
| 🤖 AI 补全卡片 | 新建角色/设定时输入名称，AI 读大纲自动填写完整信息 |
| ⏹ 中断生成 | 大纲/卷纲/细纲/正文/校对/卡片生成 全部支持一键取消 |
| 🔧 取消提示统一 | 取消后显示"已取消生成"而非"生成失败" |
| 🗑 删除 Token 监控 | 移除不准确的 Token 统计 UI |
| 📏 段落配比 | 风格库分析长短段比例，AI 参考配比生成长短交错的段落 |
| 📋 MD 导出 | 新增 Markdown 导出 |
| 🔧 大量修复 | JSON 解析容错、表单不填充、白屏、递归 bug 等 |
