...
每个层级读取上一级输出。**大纲/卷纲**参考拆文库（套路结构）和设定库（角色/世界观）；**正文**参考风格库（句式/氛围）和人格库（情感/冲突/人情/语言/读者关系）。正文额外读取上一章结尾、记录官摘要和事实簿硬规则。

### 📐 大纲

| 项目 | 内容 |
|------|------|
| **触发** | 工具栏「🤖 大纲」→ 选择参考（拆文库+设定库，各主+辅）→ 输入提示 → 确定 |
| **System** | 7 部分结构化：分卷规划、字数规划、情绪弧线、伏笔地图、人物弧线总览、主题线与核心冲突、分章框架 |
| **输入** | 书名 + 作者想法 + 拆文库上下文 + 设定库参考 |
...
| **输入** | 大纲 + 前一卷卷纲 + 拆文库 + 设定库 + 事实簿硬规则 + 伏笔注册表状态 + 记录官前卷摘要 |
...
| 项目 | 内容 |
|------|------|
| **触发** | 工具栏「🤖 生成本章」→ 选择参考（风格库+人格库，各主+辅）→ 输入提示 → 确定 |
| **System** | 段落节奏、对话规范、去AI味(50+禁用词)、13种钩子、爽点公式 |
| **输入** | 大纲摘要 + 章细纲全部字段 + 上一章结尾(300字) + 记录官前章摘要 + 卷上下文 + 风格库 + 人格库 + 事实簿 + 作者提示 |
